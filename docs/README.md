...
| 2022.08.03 | 1.增加远程上报参数配置<br />2.增加GPS参数配置<br />3.完善蓝牙扫描参数配置 |
| 2022.08.06 | 增加蓝牙广播类型配置                                         |
| 2022.08.17 | 完善GPS配置，增加定位成功延时关闭，定位数据上报间隔，定位数据过滤时间三个参数 |
| 2022.08.24 | 增加WIFI扫描配置，WIFI围栏配置                               |
| 2022.09.18 | 1.  WIFI扫描增加使用远程上报频率标记<br />2. GPS配置增加基站定位和WIFI定位开关标记<br />3. 整理关于NB的描述 |
| 2023.05.19 | 1. 扩展短消息类型                                            |
...
| 0x80 | 蓝牙主机扫描设置 | 26B | 具体内容参考：蓝牙主机扫描设置参数表 |
| 0x81 | 蓝牙主机扫描设定围栏 | 38B | 具体内容参考：蓝牙/WIFI围栏设置参数表 |
| 0x82 | GPS定位设置 | 39B | 具体内容参考：GPS定位设置参数表 |
| 0x83 | 远程上报频率设置 | 41B | 具体内容参考：远程上报频率设置参数表 |
| 0x84 | 广播间隔时间设置 | 3B | 运动广播间隔+静止广播间隔+SOS广播间隔***\*默认33,100,33\****单位10ms |
| 0x85 | WIFI扫描设置 | 21B | 具体内容参考：WIFI扫描设置参数表 |
| 0x86 | WIFI围栏设置 | 38B | 具体内容参考：蓝牙/WIFI围栏设置参数表 只支持NB设备 |
...
| 扫描功能标记 | 4B   | 32位整数，低字节在前<br />0x00000001上报扫描数据<br />0x00000002 扫描结束立即上传数据<br />0x00000004 不佩戴不开启扫描<br />0x00000008 睡眠状态不扫描<br />0x00000010 使用远程上报频率<br />0x80000000 下发存储标记【用于收到后是否存储】 |
...
| 功能字段       | 大小 | 说明                                                         |
| -------------- | ---- | ------------------------------------------------------------ |
| 正常上报频率   | 1B   | 单位分钟 **CAT1设备默认10，NB设备默认5**                     |
...
| 关闭时段控制   | 20B  | 2B一组，可设置10组时间段，例如0x04 0x05 0x08 0x09表示4点到5点，8点到9点都关闭远程上报 |
...
| 扫描开关     | 1B   | 0x00：关闭   0x01：开启                                      |
| 围栏开关     | 1B   | 用一个字节表示7个围栏的开关状态                              |
| 扫描功能标记 | 4B   | 32位整数，低字节在前<br />0x00000001上报扫描数据<br />0x00000002 扫描结束立即上传数据<br />0x00000004 只扫描beacon设备<br />0x00000008 只扫描我司设备b6,b7,b8<br />0x00000010 只扫描特定设备，filter_name过滤<br />0x00000020 只扫描一定范围内的设备 filter_rssi过滤<br />0x00000040 特定围栏，匹配filter_name则触发围栏<br />0x00000080 校时标记[仅支持我司网关校时]<br />0x00000100 扫描For围栏<br />0x80000000 下发存储标记【用于收到后是否存储】 |
| 扫描周期     | 2B   | 16位整数 低字节在前扫描周期 单位秒 范围10-3600，**默认300**  |
| 确认扫描周期 | 1B   | 用于状态切换后确认状态的扫描周期 单位秒 范围 5-255，**默认5** |
...
| ------------ | ---- | ------------------------------------------------------------ |
| 围栏编号     | 1B   | 编号范围1-7                                                  |
| 围栏功能     | 1B   | 0x01 进入围栏关闭通信模块，离开则开启【根据设置】<br />0x02 上报进入围栏事件<br />0x04 上报离开围栏事件<br />0x08 进入围栏关闭GPS，离开则开启【根据设置】 |
| 围栏设备数量 | 1B   | 范围0-5                                                      |
| 围栏设备     | 5*7  | MAC地址+匹配长度                                             |
...
| 功能标记         | 4B   | 0x00000001:定位完成立即上报<br />0x00000002:缓存等待远程定时上报<br />0x00000004:缓存区满了立即上报<br />0x00000008:使用远程上报频率<br />0x00000010:睡眠状态下不开启定位【控制GPS和基站，WIFI定位】<br />0x00000020:不佩戴不开启定位<br />0x00000040:开启基站定位<br />0x00000080:开启WIFI定位<br />0x80000000:下发数据保存标记 |
| 定位超时时间     | 2B   | 单位秒 范围60-600，**默认120**                               |
| 信号差超时时间   | 1B   | 单位秒 范围10-255，**默认30**                                |
| 定位成功延时时间 | 2B   | 用于过滤定位数据 单位秒 范围20-3600，**默认20**              |
| 定位数据上报间隔 | 1B   | 上报定位点时间间隔 单位秒 范围1-60，**默认10**               |
| 过滤数据时间     | 1B   | 过滤开始定位成功后一段时间的数据 单位秒 范围1-60，**默认10** |
...
| **载荷实体** |    xx    |    1     | type: 0x01                               |
|              |          |    1     | 索引参数                                 |
|              |          |    n     | 1B(运动类型) + 38B(运动数据) 详情见-表 9 |
|              |          |          | 1B(运动类型) + 38B(运动数据) 详情见-表 9 |
|              |          |          | …                                        |
|  **校验码**  |    CS    |    1     |                                          |
|   **包尾**   |   0x16   |    1     |                                          |
...
| 运动类型     | 0x01 - 徒步 <br />0x02 - 跑步 <br />0x03 - 爬山 <br />0x04 - 球类运动 <br />0x05 - 力量训练 <br />0x06 - 有氧运动 <br />0x07 - 跑步机 <br />0x08 - 跳绳 <br />0x09 - 室内自行车 <br />0x0A - 山地自行车 <br />0x0B - 高强度训练 <br />0x0C - 自由训练 <br />0x0D - 游泳<br />0x0E - 瑜伽 <br />0xFE - 自定义运动 |
| ------------ | ------------------------------------------------------------ |
| **运动数据** | [1,4]     : 运动时间(本地时间)<br />[5,8]     : 运动时长(秒) <br />[9,12]   : 步数 <br />[13,16] : 距离(米)<br />[17,18] : 热量消耗(Kcal)  <br />[19]      : 最大心率  <br />[20]      : 平均心率 <br />[21,22] : 平均配速(s/km) <br />[23]      : 游泳类型(仅游泳运动) (1: 蛙泳, 2: 蝶泳, 3: 仰泳, 4: 自由泳) <br />[24,25] : 总趟数 (仅游泳运动)<br />[26,27] : 总划水次数(仅游泳运动) <br />[28,29] : 平均划水频率(strokes/min)(仅游泳运动) <br />[30,31] : 跳绳次数(仅跳绳运动) <br />[32,33] : 最大耗氧量(仅跑步运动) <br />[34,38] : 保留(FF)  <br />注:   若某字段数据字节均为0xFF则表明该运动类型无此项参数 |
...
|              |          |    1     | 包类型值固定为4，表示包头                                    |
|              |          |    1     | 运动类型  <br />0X01: 徒步 <br />0X02: 跑步  <br />0X03: 爬山 <br />0X04: 球类运动 <br />0X05: 力量训练<br />0X06: 有氧运动<br />------------<br />**B10新增类型  <br />0x07: 跑步机  <br />0x08: 跳绳 <br />0x09: 室内自行车  <br />0x0a: 山地自行车 <br />0x0b: 高强度训练  <br />0x0c: 自由训练  <br />0x0d: 游泳  <br />0x0e: 瑜伽** <br />------------<br />0XFE: 自定义运动 |
|              |          |    4     | 消耗热量,单位KCal，低字节先传                                |
|              |          |    4     | 总步数，单位步，低字节先传                                   |
|  **校验码**  |    CS    |    1     |                                                              |
|   **包尾**   |   0x16   |    1     |                                                              |
