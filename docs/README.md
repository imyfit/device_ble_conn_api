...
| 2025-11-14 | - 新增SOS紧急电话配置接口，见 **'0x02 - 参数设置与读取'**    |
...
| key         | type   | 描述             | 属性 |
| ----------- | ------ | ---------------- | ---- |
| call_number | string | 紧急联系电话号码 | 可选 |
...
| key         | type   | 描述                                                         |
| ----------- | ------ | ------------------------------------------------------------ |
| battery     | string | 电池信息："percent,charge"<br />percent - 电量百分比（0~100）<br />charge - 充电状态（0 未充电，1 充电中，2 已充满） |
| basic_data  | string | 基础数据："steps,calories,distance"<br />steps - 计步（step）<br />calories - 卡路里（Kcal）<br />distance - 里程（meters） |
| heart       | int    | 心率值（bpm）                                                |
| rri         | int    | 心跳间隔（ms）                                               |
| spo2        | int    | 血氧                                                         |
| bp          | string | 血压："sys,dia"<br />sys - 收缩压<br />dia - 舒张压          |
| temperature | string | 温度数据："hand,env,body"；仅填充支持的温度数据，不支持的将为空<br />hand - 体表温度/手臂温度 x100（摄氏度℃）<br />env - 环境温度 x100（摄氏度℃）<br />body - 体核温度 x100（摄氏度℃），此数据受环境和佩戴松紧影响 仅供参考 |
| baro        | int    | 气压（Pa）                                                   |
...
| active_time     | int  | 累计活动时间（second）     |
| silent_time     | int  | 累计静止时间（second）     |
...
> 数据存储间隔为 1 分钟
>
> 注：需要开启[GNSS监测](#0x02 - 参数设置与读取)功能才会有相应的历史数据
...
|              |      | 2    | 位置数据包索引值。                                           |
|              |      | 2    | 位置数据包总数。                                             |
|              |      | 12   | 4B(经度 x1e6) + 4B(纬度 x1e6)  + 1B(信号强度 0~100) + 3B(预留) |
|              |      | 12   | 同上                                                         |
|              |      | 12   | 同上                                                         |
|              |      | ...  | ...                                                          |
|  **校验码**  | CS   | 1    |                                                              |
...
|              |      | 2    | HRV2数据包索引值。                                           |
|              |      | 2    | HRV2数据包总数。                                             |
|              |      | 8    | 1B(精神压力) + 1B(疲劳度) + 1B(抗压能力) + 1B(调节能力) + 4B(保留 0) |
|              |      | 8    | 同上                                                         |
|              |      | 8    | 同上                                                         |
|              |      | ...  | ...                                                          |
|  **校验码**  | CS   | 1    |                                                              |
