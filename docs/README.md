...
| 版本  |   日期   |  作者  |                      备注                      |
| :---: | :------: | :----: | :--------------------------------------------: |
| 4.0.1 | 20220413 | 吴华志 |   对该版本之前的所有版本进行整理，形成此文档   |
| 4.0.2 | 20220416 | 吴华志 | 增加标签终端配置协议(0x3D)，添加声光报警器配置 |
...
| 包头  (1Byte) | 控制码   (1Byte) | 数据长度  (2Byte) | 内容数据  (n*Byte) | CRC8  (1Byte) | 包尾  (1Byte) |
| :-----------: | :--------------: | :---------------: | :----------------: | :-----------: | :-----------: |
|      68H      |        …         |   内容数据长度    |         …          |       …       |      16H      |

1. **包头**，固定0x68。

2. **控制码**，各部分位含义如下表。部分控制码可能与该定义冲突，以实际为准。详细的控制码（也称为：功能码、功能协议）在后文详细阐述。
   ...
   如果终端回复的控制码是异常码，则数据域会有异常参数（一个字节），如下表。
...
3. **数据长度**，为两个字节，低字节在前，表征**内容数据**域的字节长度。

4. **内容数据**，具体的数据内容，在后文详细阐述。

5. **CRC8**，将包头、控制码、数据长度、内容数据所有字节进行相加取低字节。
